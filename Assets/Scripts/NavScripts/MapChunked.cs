...
public class MapChunked {
    ...
    public NavTri triFromPoint(Vector2 p) {
        uint x = (uint)(p.x / MeshChunk.ChunkSize);
        uint y = (uint)(p.y / MeshChunk.ChunkSize);

        if (x > navMesh.GetLength(0) || y >= navMesh.GetLength(1)) {
            Debug.LogError("tri point out of range");
            return null;
        }

        foreach(NavTri t in navMesh[x,y].tris) {
            if (NavCalc.PointInTri(p,t))
                return t;
        }
